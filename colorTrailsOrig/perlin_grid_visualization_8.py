...
def inject_noise_emitter(surface: pygame.Surface, t: float, endpoint_speed: float, color_shift: float) -> None:
    # 2D noise emitter on the full 32x32 grid.
    # Render only one quarter and mirror it kaleidoscopically across both axes.
    # Antialiased/subpixel-like deposition from a continuous noise field.
    # Draw only raw noise values in [0.0, +0.2] and map to color profile.
    scale = 0.0375
    drift_x = t * 0.55 * endpoint_speed
    drift_y = -t * 0.43 * endpoint_speed
    layer2_drift_x = -t * 0.34 * endpoint_speed + 9.7
    layer2_drift_y = t * 0.61 * endpoint_speed + 4.1
    # Base patch: rows/cols 0..16 (17 cells). Mirror-copy only 15 rows/cols.
    # This avoids doubling on the center seam.
    base = (GRID_SIZE // 2) + 1  # 17 for a 32x32 grid
    mirror_limit = base - 2      # 15 -> only indices 0..14 are mirrored

    def emit_layer(off_x: float, off_y: float, base_phase: float, neighbor_span: float) -> None:
        h0 = (t * color_shift + base_phase) % 1.0
        h1 = (h0 + neighbor_span) % 1.0
        for y in range(base):
            for x in range(base):
                sx = clamp(x + 0.37, 0.0, (base - 1) - 1e-6)
                sy = clamp(y + 0.29, 0.0, (base - 1) - 1e-6)
                n = fbm2d_hq(sx * scale + off_x, sy * scale + off_y)
                if 0.0 <= n <= 0.2:
                    u = n / 0.2
                    tprof = 1.0 - abs(u - 0.5) * 2.0
                    tprof = tprof ** 0.65
                    gain = clamp(0.80 + 0.20 * tprof, 0.0, 1.0)
                    # Per-layer palette: base hue fades to adjacent hue across the local noise range.
                    hm = lerp_hue(h0, h1, u)
                    r_f, g_f, b_f = colorsys.hsv_to_rgb(hm, 1.0, gain)
                    ...
                    fy = sy - yi
                    taps = [
                        (int(xi), int(yi), (1.0 - fx) * (1.0 - fy)),
                        (int(xi + 1), int(yi), fx * (1.0 - fy)),
                        (int(xi), int(yi + 1), (1.0 - fx) * fy),
                        (int(xi + 1), int(yi + 1), fx * fy),
                    ]
                    for tx, ty, w in taps:
                        if w <= 0.0:
                            continue
                        ...
                        my = GRID_SIZE - 1 - ty
                        targets = {(tx, ty)}
                        if 0 <= tx <= mirror_limit:
                            targets.add((mx, ty))
                        if 0 <= ty <= mirror_limit:
                            targets.add((tx, my))
                        if 0 <= tx <= mirror_limit and 0 <= ty <= mirror_limit:
                            targets.add((mx, my))
                        for px, py in targets:
                            blend_pixel_weighted(surface, px, py, color, w)

    emit_layer(drift_x, drift_y, 0.00, +0.16)
    emit_layer(layer2_drift_x, layer2_drift_y, 0.48, -0.18)
...
def main() -> None:
    ...
    win_w = max(960, int(display_info.current_w * 0.9))
    win_h = max(720, int(display_info.current_h * 0.9))
    ...
    panel_w = max(360, min(560, int(screen_w * 0.30)))
    ui_x = screen_w - panel_w - MARGIN
    grid_pixels = min(screen_h - 2 * MARGIN, ui_x - 2 * MARGIN)
    ...
    panel_rect = pygame.Rect(ui_x, MARGIN, panel_w, screen_h - 2 * MARGIN)

    emitter_gap = 6
    emitter_btn_w = (panel_w - 40 - emitter_gap * 4) // 5
    ...
    emitter_noise_button = Button("Noise", pygame.Rect(ui_x + 20 + (emitter_btn_w + emitter_gap) * 4, MARGIN + 58, emitter_btn_w, 28), active=False)

    mode_gap_x = 8
    mode_gap_y = 4
    mode_cols = 2
    mode_btn_h = 18
    mode_btn_w = (panel_w - 40 - mode_gap_x) // mode_cols
    mode_rows = (len(MODE_BUTTON_LABELS) + mode_cols - 1) // mode_cols
    blur_h = 24
    mode_grid_y = panel_rect.bottom - 12 - blur_h - 6 - mode_rows * (mode_btn_h + mode_gap_y)
    mode_grid_y = max(mode_grid_y, panel_rect.top + 150)

    slider_start_y = emitter_line_button.rect.bottom + 22
    slider_gap = 42
    if slider_start_y + 2 * slider_gap + 14 > mode_grid_y - 22:
        slider_gap = max(30, int((mode_grid_y - 22 - slider_start_y - 14) / 2))

    def slider_y(idx: int) -> int:
        return slider_start_y + idx * slider_gap

    sliders = [
        Slider("Endpoint Speed", 0.00, 2.00, ENDPOINT_SPEED_DEFAULT, pygame.Rect(ui_x + 20, slider_y(0), panel_w - 40, 14), 2),
        Slider("Color Cycling Speed", 0.00, 1.00, COLOR_SHIFT_DEFAULT, pygame.Rect(ui_x + 20, slider_y(1), panel_w - 40, 14), 2),
        Slider("Fade %", 95.0, 100.0, 99.995, pygame.Rect(ui_x + 20, slider_y(2), panel_w - 40, 14), 3),
    ]
    ...
    blur_button = Button("Blur", pygame.Rect(ui_x + 20, mode_grid_y + mode_rows * (mode_btn_h + mode_gap_y) + 6, panel_w - 40, blur_h), active=False)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if emitter_line_button.handle_event(event):
                ...
                emitter_noise_button.active = False
            ...
            if emitter_ring_button.handle_event(event):
                ...
                emitter_noise_button.active = False
            if emitter_noise_button.handle_event(event):
                emitter_mode = "noise"
                emitter_line_button.active = False
                emitter_dots_button.active = False
                emitter_triangle_button.active = False
                emitter_ring_button.active = False
                emitter_noise_button.active = True
        ...
        elif emitter_mode == "ring":
            inject_bouncing_ring(grid_surface, t, color_shift, endpoint_speed)
        else:
            inject_noise_emitter(grid_surface, t, endpoint_speed, color_shift)
        ...
        screen.fill(BG)
        scaled_grid = pygame.transform.scale(grid_surface, (grid_rect.width, grid_rect.height))
        ...
        emitter_noise_button.draw(screen, font)
